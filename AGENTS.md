...
- **Personal Mobile Pipeline** (`BierDav/noodle-gallery` fork only, not upstream): a `quickme-branding` bookmark carries a small personal patch (branding overlay) rebased directly on top of the fork's `release` tag, built into a sideloadable APK that Obtainium auto-installs. All of it — workflow, docs, branding overlay — lives on `quickme-branding` only, never on `main`; `main` tracks upstream and should never carry fork-personal-only content. **If CI here fails or "works on my machine" for something touching `quickme-branding`, suspect the rebase first**: every run of `.github/workflows/personal-mobile-pipeline.yml` re-rebases `quickme-branding` onto whatever `release` currently points to (locally, never pushed automatically — force-pushing a resolved rebase is a manual, human step), so the commit actually being built is rarely the exact commit you last looked at locally unless you just force-pushed it yourself. A `schedule` trigger only fires from the workflow file on the default branch, so the daily poll for `release` moving lives in a separate minimal stub, `.github/workflows/personal-mobile-pipeline-trigger.yml` on `main`, whose only job is to dispatch the real pipeline on `quickme-branding` — don't mistake it for the pipeline itself. See `docs/personal-mobile-pipeline.md`.
